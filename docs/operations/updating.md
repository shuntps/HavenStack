...
Merging a pull request only changes `main`. Pushing a `v*` tag creates a GitHub Release after the same Compose validation; it still does not deploy the hosts.
